        ...
        <div>
            @if ($myRequests->isEmpty())
                <div class="empty-state">
                    ...
                    <a href="{{ route('portal.register-street') }}" class="btn btn-primary"
                        style="margin-top:16px;"><i class="fas fa-plus"></i> Register a Street</a>
                </div>
            @else
                        ...
                        <tbody>
                            @foreach ($myRequests as $req)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="status-badge {{ $req->status }}">{{ str_replace('_', ' ', ucfirst($req->status)) }}</span>
                                        @if ($req->status === 'awaiting_payment')
                                            <button wire:click="emitInitiateStreet({{ $req->id }})"
                    ...
                    @foreach ($approved as $cert)
                        <div class="card" style="border:2px solid var(--accent);position:relative;overflow:hidden;">
                            ...
                            <button class="btn btn-outline btn-sm"
                                style="margin-top:14px;width:100%;justify-content:center;" onclick="window.print()">
