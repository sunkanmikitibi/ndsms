<div class="admin-container">
                        ...
                        <tbody>
                            @foreach ($requests as $req)
                                <tr>
                                    ...
                                    <td class="text-sm">
                                        @if ($req->assignedUser)
                                            <div class="font-semibold">{{ $req->assignedUser->name }}</div>
                                            @if ($req->estimated_completion_date)
                                                <div class="text-xs text-gray-500">Est:
                                                    {{ $req->estimated_completion_date->format('M d') }}</div>
                                            @endif
                                        @else
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            ...
                                            @elseif ($req->status === 'approved')
                                                <button wire:click="openAssignModal({{ $req->id }})"
                                                    class="btn btn-sm btn-primary" title="Assign to Production">
                                                    <i class="fas fa-user-plus"></i>
                                                </button>
                                            @elseif (in_array($req->status, ['in_production', 'ready', 'delivered']))
                                                <button wire:click="openStatusModal({{ $req->id }})"
                                                    class="btn btn-sm btn-info" title="Update Status">
        ...
        <div class="modal-overlay" @click="$wire.closeDetail()">
            <div class="modal-content" @click.stop>
                ...
                <div class="modal-body">
                    ...
                    @if ($selectedRequest->assigned_to || $selectedRequest->production_started_at)
                        <div class="detail-section">
                            <h3>Production Information</h3>
                            <div class="detail-grid">
                                @if ($selectedRequest->assignedUser)
                                    <div class="detail-item">
                                        <span class="detail-label">Assigned To</span>
                                        <span class="detail-value">{{ $selectedRequest->assignedUser->name }}</span>
                                    </div>
                                @endif
                                @if ($selectedRequest->estimated_completion_date)
                                    <div class="detail-item">
                                        <span class="detail-label">Est. Completion</span>
                                        <span
                                            class="detail-value">{{ $selectedRequest->estimated_completion_date->format('F j, Y') }}</span>
                                    </div>
                                @endif
                                @if ($selectedRequest->production_started_at)
                                    <div class="detail-item">
                                        <span class="detail-label">Production Started</span>
                                        <span
                                            class="detail-value">{{ $selectedRequest->production_started_at->format('F j, Y \a\t H:i') }}</span>
                                    </div>
                                @endif
                                @if ($selectedRequest->production_completed_at)
                                    <div class="detail-item">
                                        <span class="detail-label">Production Completed</span>
                                        <span
                                            class="detail-value">{{ $selectedRequest->production_completed_at->format('F j, Y \a\t H:i') }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>
                    ...
                    <!-- Production Notes -->
                    @if ($selectedRequest->production_notes)
                        <div class="detail-section">
                            <h3>Production Notes</h3>
                            <div class="note-box">{{ $selectedRequest->production_notes }}</div>
                        </div>
                    @endif
    ...
    @if ($showAssignModal && $selectedRequest)
        <div class="modal-overlay" @click="$wire.set('showAssignModal', false)">
            <div class="modal-content" @click.stop style="max-width: 600px;">
                <div class="modal-header">
                    <h2>Assign to Production Team</h2>
                    <button type="button" wire:click="$set('showAssignModal', false)" class="btn-close">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                ...
                <div class="modal-body">
                    <p class="mb-4">
                        Assign this numbering plate request to a production team member:
                    </p>
                    <div class="info-box">
                        <p><strong>Reference:</strong> {{ $selectedRequest->reference_number }}</p>
                        <p><strong>Street:</strong> {{ $selectedRequest->street_name }}</p>
                    ...
                    <div class="form-group">
                        <label>Production Team Member *</label>
                        <select wire:model="assignedUserId" class="form-control">
                            <option value="">Select team member...</option>
                            @foreach ($productionTeam as $user)
                                <option value="{{ $user->id }}">{{ $user->name }}
                                    ({{ $user->roles->pluck('name')->join(', ') }})</option>
                            @endforeach
                        </select>
                        @if (!$assignedUserId)
                            <span style="color: #dc2626; font-size: 12px;">Production team member is required</span>
                        @endif
                    </div>

                    <div class="form-group">
                        <label>Estimated Completion Date</label>
                        <input type="date" wire:model="estimatedCompletionDate" class="form-control"
                            min="{{ now()->format('Y-m-d') }}">
                    </div>

                    <div class="form-group">
                        <label>Production Notes</label>
                        <textarea wire:model="productionNotes" rows="3" placeholder="Any special instructions or notes..."
                            class="form-control"></textarea>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    <button type="button" wire:click="$set('showAssignModal', false)"
                        class="btn btn-outline">Cancel</button>
                    <button type="button" wire:click="assignToProduction" class="btn btn-primary"
                        {{ !$assignedUserId ? 'disabled' : '' }}>
                        <i class="fas fa-user-plus"></i> Assign to Production
                    </button>
                </div>
            </div>
        </div>
    ...
    <!-- Status Update Modal -->
    @if ($showStatusModal && $selectedRequest)
        <div class="modal-overlay" @click="$wire.set('showStatusModal', false)">
            <div class="modal-content" @click.stop style="max-width: 500px;">
                <div class="modal-header">
                    <h2>Update Request Status</h2>
                    <button type="button" wire:click="$set('showStatusModal', false)" class="btn-close">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
                ...
                <div class="modal-body">
                    <p class="mb-4">
                        Update the status for this numbering plate request:
                    </p>
                    <div class="info-box">
                        <p><strong>Reference:</strong> {{ $selectedRequest->reference_number }}</p>
                        <p><strong>Current Status:</strong> {{ $statuses[$selectedRequest->status] }}</p>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select wire:model="newStatus" class="form-control">
                            ...
                            @elseif ($selectedRequest->status === 'in_production')
                                <option value="ready">Ready for Delivery</option>
                            @elseif ($selectedRequest->status === 'ready')
                                <option value="delivered">Delivered</option>
                            @elseif ($selectedRequest->status === 'delivered')
                                <option value="installed">Installed</option>
                            @elseif ($selectedRequest->status === 'installed')
                ...
                <div class="modal-footer">
                    <button type="button" wire:click="$set('showStatusModal', false)"
                        class="btn btn-outline">Cancel</button>
                    <button type="button" wire:click="updateStatus" class="btn btn-info"
                        {{ !$newStatus ? 'disabled' : '' }}>
